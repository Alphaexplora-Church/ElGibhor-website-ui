...
const getJson = async <T>(path: string, fallback: string): Promise<T> => {
  const response = await fetch(`${API_BASE_URL}/api/journeys/public${path}`);
  if (!response.ok) {
...
export const JourneysService = {
  fetchCategories: async (): Promise<JourneyCategory[]> => {
    const body = await getJson<{ categories?: JourneyCategory[] }>('/categories', 'Failed to load topics');
    return [...(body.categories ?? [])].sort((a, b) => a.sortOrder - b.sortOrder);
  },

  fetchJourneys: async ({ page = 1, limit = 8, category }: JourneyListQuery = {}): Promise<JourneyPage> => {
